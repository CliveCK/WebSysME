...
#Region "Retrieve Overloads" 
    ...
    Public Overridable Function GetGroupMaturityIndex(ByVal GroupID As Long) As DataSet
        ...
        If GroupID > 0 Then
            sql = "SELECT G.GroupMaturityIndexID, M.Description as [Month], [Year], A.Description as KeyArea, Score FROM tblGroupMaturityIndex G "
            sql &= "inner join luKeyAreas A On G.KeyAreaID = A.KeyAreaID "
            sql &= "inner join luMonths M on M.MonthID = G.MonthID WHERE GroupMaturityIndexID = " & GroupID
        Else
            sql = "SELECT G.GroupMaturityIndexID, M.Description as [Month], [Year], A.Description as KeyArea, Score FROM tblGroupMaturityIndex G "
            sql &= "inner join luKeyAreas A On G.KeyAreaID = A.KeyAreaID "
            sql &= "inner join luMonths M on M.MonthID = G.MonthID WHERE GroupMaturityIndexID = " & mGroupID
        End If
