...
#Region "Retrieve Overloads" 
    ...
    Public Function GetAllPatients() As DataSet
        ...
        sql = "select P.*, R.Description As Religion, O.Description As Orphanhood, S.Name As School, H.Name As ClosestHealthCenter, M.Description As MedicalAidType "
        sql &= " from tblPatients P "
        sql &= "inner join luReligion R on P.ReligionID = R.ReligionID "
        sql &= "inner join luMedicalAidTypes M on M.MedicalAidTypeID = P.MedicalAidTypeID "
        sql &= "inner join tblOrphanhood O on O.ObjectID = P.OrphanhoodID "
        sql &= "inner join tblSchools S on S.SchoolID = P.SchoolID "
        sql &= "inner join tblHealthCenters H on H.HealthCenterID = P.ClosestHealthCenterID"
